...
class AnimatedWidgetBuilderData {
  ...
  const AnimatedWidgetBuilderData(
    this.animation, {
...
class _UpdateFlags {
  const _UpdateFlags([int flags = 0])
      : _value = flags,
        ...
        assert(
          (flags & (CLEAR_LAYOUT_OFFSET | KEEP_FIRST_LAYOUT_OFFSET)) !=
              KEEP_FIRST_LAYOUT_OFFSET,
        ),
        assert(
...
_IntervalBuilder? offsetIntervalBuilder(
  final _IntervalBuilder? iBuilder,
  final int offset,
) {
...
_IntervalBuilder joinBuilders(
  final _IntervalBuilder leftBuilder,
  final _IntervalBuilder rightBuilder,
  final int leftCount,
) {
  assert(leftCount > 0);
